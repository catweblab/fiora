...
<div id="sagepay" class="content">
 <table width="100%">
  ...
  <tr>
   ...
   <td>
    <select name="cc_type">
     <?php foreach ($cards as $card) { ?><option value="<?php echo $card['value']; ?>"><?php echo $card['text']; ?></option><?php } ?>
    </select>
   </td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td><input type="text" name="cc_issue" value="" size="1" /> <?php echo $text_issue; ?></td>
  </tr>
 </table>
</div>
<div class="buttons">
...
function confirmSubmit() {
	$.ajax({
		...
		beforeSend: function() {
			...
			$('#sagepay').before('<div class="wait"><img src="catalog/view/theme/clothing_store/image/loading_1.gif" alt="" /> <?php echo $text_wait; ?></div>');
		},
		success: function(data) {
			...
			$('.wait').remove();
			if (data.success) { location = data.success; }
		}
	});
}
//--></script>